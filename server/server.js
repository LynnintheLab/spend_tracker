...
import {
  makeToken, readToken, findUser, listUsers, usersFileExists,
  passcodeRequired, passcodeOk,
} from './auth.js';
...
async function handleApi(req, res, url) {
  ...
  // Booleans only — never the names or the value itself. usersEnvPresent
  // separates "the panel never passed USERS to the process" from "it did, but
  // the value is malformed", which need opposite fixes.
  if (route === '/api/config') {
    return sendJson(res, 200, {
      passcodeRequired: passcodeRequired(),
      usersConfigured: listUsers().length > 0,
      usersEnvPresent: Boolean(process.env.USERS),
      usersFilePresent: usersFileExists(),
    });
  }
